from trading1.strategy.base_strategy import Strategy
...
from trading1.pusher.slack import PushSlack
import configparser, datetime, traceback, sys
from trading1.logger import get_logger
import redis
...
class StepTrade(Strategy):
    ...
    def scenario(self):
        ...
        my_orders = self.db_handler.find_items({"currency": self.currency_type,
                                                "$or": [{"status": "BUY_ORDERED"}, {"status": "SELL_ORDERED"},
    ...
    def check_buy_ordered(self):
        buy_orders = self.db_handler.find_items({"currency": self.currency_type, "status": "BUY_ORDERED"}, "trader",
                                                "trade_status")
        ...
        for item in buy_orders:
            ...
            if len(order_result) > 0 and order_result[0]["status"] == "filled" and order_result[0]["price"] == str(
                    item["buy"]):
            ...
            elif len(order_result) > 0 and order_result[0]["status"] == "unfilled" and order_result[0]["price"] == str(
                    item["buy"]):
                if int(item["buy"]) + int(self.params["step_value"]) <= self.last_val:
                    logger.info("CancelOrder")
                    logger.info(item)
                    try:
                        self.order_cancel_transaction(machine=self.machine, db_handler=self.db_handler,
                                                      currency_type=self.currency_type, item=item)
                    except:
    ...
    def check_buy_completed(self):
        buy_completed = self.db_handler.find_items({"currency": self.currency_type, "status": "BUY_COMPLETED"},
                                                   "trader", "trade_status")
        logger.info("BUY_COMPLETED")
        for item in buy_completed:
            logger.info(item)
            try:
                self.order_sell_transaction(machine=self.machine, db_handler=self.db_handler,
                                            currency_type=self.currency_type, item=item)
    ...
    def check_sell_ordered(self):
        sell_orders = self.db_handler.find_items({"currency": self.currency_type, "status": "SELL_ORDERED"}, "trader",
                                                 "trade_status")
    ...
    def check_sell_completed(self):
        sell_completed = self.db_handler.find_items(
            {"currency": self.currency_type, "$or": [{"status": "SELL_COMPLETED"}, {"status": "CANCEL_ORDERED"}]},
            "trader", "trade_status")
    ...
    def check_keep_ordered(self):
        keeped_orders = self.db_handler.find_items({"currency": self.currency_type, "status": "KEEP_ORDERED"}, "trader",
                                                   "trade_status")
